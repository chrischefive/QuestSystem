...
namespace Sisus
{
	...
	public sealed class DefaultDrawerProvider : DrawerProviderBase
	{
		...
		{
			#if DEV_MODE
			Debug.Log("DefaultDrawerProvider constructor");
			#endif
		...
		#endif
		{
			...
			#if DEV_MODE
			Debug.Log("DefaultDrawerProvider.SetupThreaded finished !");
			#endif
		...
		{
			return TypeExtensions.GetAllTypesThreadSafe(false, false, true).Where(IsDrawerType);
		}
		...
		private void BuildDictionariesForExactTypes(ThreadSafeSetupData drawersFor, Type drawerType, bool isFallback)
		{
			...
			if(!Attribute<DrawerForBaseAttribute>.TryGetAll(drawerType, out attributes))
			{
				return;
			}
			...
			foreach(var attribute in attributes)
			{
				...
				if(fieldAttribute != null)
				{
					...
					for(int t = subjectTypes.Length - 1; t >= 0; t--)
					{
						...
						{
							Debug.Log(subjectType.Name + " handled by "+drawerType.Name+" because it's a base type of "+type.Name);
						}
						#endif
