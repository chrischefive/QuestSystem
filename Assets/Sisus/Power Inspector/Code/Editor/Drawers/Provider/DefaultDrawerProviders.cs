	...
	public static class DefaultDrawerProviders
	{
		...
		public static void Deserialize()
		{
			...
			try
			{
				...
				UnityEngine.Debug.Assert(drawerProvidersByInspectorType.TryGetValue(typeof(PowerInspector), out found), "Drawer provider for Power Inspector not found");
		...
		#endif
		{
			...
			#if !CSHARP_7_3_OR_NEWER
			SetupTypesInParallel(TypeExtensions.GetAllTypesThreadSafe(false, true, true));
			#else
			await SetupTypesInParallel(TypeExtensions.GetAllTypesThreadSafe(false, true, true));
			#endif
			...
			#if DEV_MODE && DEBUG_SETUP
			UnityEngine.Debug.Log("DefaultDrawerProviders.threadedFullRebuildFinished = true");
			#endif
