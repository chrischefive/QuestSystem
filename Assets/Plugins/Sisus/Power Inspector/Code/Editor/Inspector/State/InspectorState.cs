...
namespace Sisus
{
	...
	public class InspectorState
	{
		...
		public void UpdateDimensions(Rect inspectorWindowRect, float toolbarHeight, float previewAreaHeight, IInspector drawOnScreen = null)
		{
			...
			// Repeat redrawing until widths and height are no longer changing?
			// UPDATE: This doesn't seem to be of any help. Dimensions never seem to change a second time.
			//if((widthChanged || heightChanged) && drawOnScreen != null)
			//{
			//	UpdateDimensions(inspectorWindowRect, toolbarHeight, previewAreaHeight, drawOnScreen);
			//}
		}

		public bool CanHaveScrollBar()
		{
			return drawers.Length > 0 && !(drawers[0] is IScrollable);
		}
