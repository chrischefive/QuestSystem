...
namespace Sisus
{
	...
	public static class TypeExtensions
	{
		...
		public static HashSet<Type> InvalidComponentTypes
		{
			get
			{
				return new HashSet<Type>(WhereEditorOnly(GetAllAssembliesThreadSafe()).SelectMany((assembly) => assembly.GetLoadableTypes(false).Where((type)=>type.IsComponent())));
			}
		}
		...
		public static IEnumerable<Type> GetAllTypesThreadSafe(bool includeAbstract = true, bool includeInvisible = false, bool includeDuplicates = true)
		{
			...
			if(includeAbstract)
			{
				return GetAllAssembliesThreadSafe().SelectMany((assembly) => assembly.GetLoadableTypes(!includeInvisible));
			}

			return GetAllAssembliesThreadSafe().SelectMany((assembly) => assembly.GetLoadableTypes(!includeInvisible).Where(TypeIsConcrete));
		}

		public static List<Type> GetExtendingTypesThreadSafeExceptionSafeSlow(Type baseType, bool includeAbstract = true, bool includeInvisible = false, bool includeDuplicates = true)
		{
			var allTypes = GetAllTypesThreadSafe(includeAbstract, includeInvisible, includeDuplicates);
		...
		public static IEnumerable<Type> GetAbstractTypesThreadSafe(bool includeInvisible = false)
		{
			if(includeInvisible)
			{
				GetAllAssembliesThreadSafe().SelectMany((assembly) => assembly.GetLoadableTypes(false).Where(TypeIsAbstract));
			}
			return GetAllAssembliesThreadSafe().SelectMany((assembly) => assembly.GetLoadableTypes(false).Where(TypeIsVisibleAndAbstract));
		}
