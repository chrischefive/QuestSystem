...
namespace Sisus.CreateScriptWizard
{
	public class CreateScriptWizardWindow : EditorWindow
	{
		...
		private bool IsCustomEditorTemplateSelected
		{
			get
			{
		...
		private void OnTemplateChanged()
		{
			...
			if(isEditorClass && !FileUtility.IsEditorPath(directory))
			{
		...
		private void OnEnable()
		{
			...
			#if DEV_MODE
			Debug.Log($"directoryBrowseIcon: {directoryBrowseIcon.image}.");
			#endif
		}
		...
		private void Create()
		{
			...
			EditorPrefs.SetString(ScriptBuilder.CreatedAtPath, GetTargetPath());
			...
			if(CanAddComponent())
			{
				...
				addScriptMethod.Invoke(null, new Object[] { gameObjectToAddTo, AssetDatabase.LoadAssetAtPath(GetTargetPath(), typeof(MonoScript)) as MonoScript });
			}
		...
		private void CreateSingletonBaseClass(string baseClassName)
		{
			...
			var filePath = GetTargetPath(baseClassName);
		...
		private void OpenFolderPanel()
		{
			...
			if(!string.Equals(targetDir, setTargetDir))
			{
				...
				{
					if(setTargetDir.Length <= 7)
					{
						setTargetDir = "";
					}
					else
					{
						setTargetDir = setTargetDir.Substring(7);
					}
				}
				directory = setTargetDir;
			}
		}
		...
		private bool InvalidTargetPath()
		{
			...
			if(GetTargetDir().Split(PathSeparatorChars, StringSplitOptions.None).Contains(string.Empty))
			{
				return true;
			}
		...
		{
			return GetTargetPath(scriptPrescription.className);
		}

		private string GetTargetPath(string className)
		{
			return Path.Combine(GetTargetDir(), className + ".cs");
		}
		...
		private void SetClassName(string newClassName)
		{
			if(!string.Equals(scriptPrescription.className, newClassName))
			{
				...
				scriptAtPathAlreadyExists = File.Exists(GetTargetPath());
		...
		{
			CreateScript(GetTargetPath(), code, true);
		}

		private static void CreateScript(string targetPath, string code, bool refreshAssetDatabase)
		{
		...
		{
			#if DEV_MODE && DEBUG_REBUILD_CODE
			Debug.Log("Updating code preview...");
			#endif
